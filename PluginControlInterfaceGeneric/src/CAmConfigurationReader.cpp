...
CAmConfigurationReader &CAmConfigurationReader::instance(void)
{
    // Function-scoped Singleton instance, lazy initialized
    // and destroyed on library unload
    static CAmConfigurationReader singleton;
...
void CAmConfigurationReader::reload(void)
{
    mMapSources.clear();
    mMapSinks.clear();
    mMapGateways.clear();
    ...
    mMapClasses.clear();
    mListPolicies.clear();
    mListSystemProperties.clear();
    mMapScaleConversions.clear();
    ...
    if (err == E_OK)
    {
        ...
        mMapScaleConversions  = configuration.listTemplateMapScaleConversions;
    }
}
...
void CAmConfigurationReader::populateScalingMap(std::map<float, float > &outMap, const std::string &valuePairString)
{
    CAmXmlConfigParser::populateScalingMap(outMap, valuePairString, mMapScaleConversions);
}

am_Error_e CAmConfigurationReader::getListSources(std::vector<gc_Source_s > &listSources)
{
    _getListElements(listSources, mMapSources);
    return E_OK;
}

am_Error_e CAmConfigurationReader::getListSinks(std::vector<gc_Sink_s > &listSinks)
{
    _getListElements(listSinks, mMapSinks);
    return E_OK;
}

am_Error_e CAmConfigurationReader::getListGateways(std::vector<gc_Gateway_s > &listGateways)
{
    _getListElements(listGateways, mMapGateways);
    return E_OK;
}

am_Error_e CAmConfigurationReader::getListDomains(std::vector<gc_Domain_s > &listDomains)
{
    _getListElements(listDomains, mMapDomains);
    return E_OK;
}

am_Error_e CAmConfigurationReader::getListClasses(std::vector<gc_Class_s > &listClasses)
{
    ...
    Telement &itListElement)
{
    ...
    gc_MainSoundProperty_s /*am_MainSoundProperty_s*/ mainSoundProperty;
         ...
         ++itListMainSoundProperties)
    {
        ...
        mainSoundProperty.isPersistenceSupported = itListMainSoundProperties->isPersistenceSupported;
...
void CAmConfigurationReader::_prepareSinkMap(gc_Configuration_s *pConfiguration)
{
    ...
    {
        if (itListSinks->registrationType != REG_NONE)
        {
...
void CAmConfigurationReader::_prepareSourceMap(gc_Configuration_s *pConfiguration)
{
    ...
    {
        if (itListSources->registrationType != REG_NONE)
        {
...
void CAmConfigurationReader::_prepareGatewayMap(gc_Configuration_s *pConfiguration)
{
    ...
    {
        if (itListGateways->registrationType != REG_NONE)
        {
...
void CAmConfigurationReader::_prepareDomainMap(gc_Configuration_s *pConfiguration)
{
    ...
    {
        if (itListDomains->registrationType != REG_NONE)
        {
