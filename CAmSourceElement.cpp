...
CAmSourceElement::CAmSourceElement(const gc_Source_s& sourceData,
                                ...
                                mSource(sourceData),
								 mMapUserToNormalizedVolume(
										 sourceData.mapUserVolumeToNormalizedVolume),
								                                mMapNormalizedToDecibelVolume(
								                                		sourceData.mapNormalizedVolumeToDecibelVolume),
                                mNumInUse(0)
{
    setType (ET_SOURCE);
...
am_mainVolume_t CAmSourceElement::convertVolumeToMainVolumeSource(const am_volume_t DBVolume)
{
    float mainVolume;
...
am_Error_e CAmSourceElement::setMuteState(const am_MuteState_e muteState)
{
    ...
    {
        return mpControlReceive->changeSourceMuteStateDB(muteState, getID());
    }
    return E_OK;
   
}

am_Error_e CAmSourceElement::getMuteState(am_MuteState_e& muteState) const
{
	 am_Source_s sourceData;
    am_Error_e result;
    //get the sink Info from Database
    result = mpControlReceive->getSourceInfoDB(getID(), sourceData);
   // muteState = sourceData.muteState;
    return result;
}
