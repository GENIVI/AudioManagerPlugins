...
am_Error_e CAmControlSend::hookUsersetSourceMuteState(const am_sourceID_t sourceID,
		const am_MuteState_e muteState)
{
    CAmElement* pElement;
    ...
    // Store the trigger in a Queue
    gc_SourceMuteTrigger_s* triggerData = new gc_SourceMuteTrigger_s;
    if (NULL == triggerData)
    {
        LOG_FN_ERROR("  bad memory state:");
        return E_NOT_POSSIBLE;
	...
	triggerData->muteState = muteState;
    //mute state will be taken care at policy send side.
    CAmTriggerQueue::getInstance()->queue(USER_SET_SOURCE_MUTE_STATE, triggerData);
    iterateActions();
    LOG_FN_EXIT();
    return (E_OK);
}
...
{
	LOG_FN_INFO(" enter into CAmControlSend::cbAckConnect() called"); 
                                                         ...
                                                         gc_TriggerElement_s* triggerData)
{
    ...
    case USER_SET_SINK_MUTE_STATE:
    {
        ...
        break;
	case USER_SET_SOURCE_MUTE_STATE:
	{
		gc_SourceMuteTrigger_s* pSourceMuteTrigger_t = (gc_SourceMuteTrigger_s*)triggerData;
		result = mpPolicySend->hookSetSourceMuteState(pSourceMuteTrigger_t->sourceName,
				pSourceMuteTrigger_t->muteState);
	}
	break;
