                                ...
                                mpPolicyReceive(NULL)
{
    ...
    mMapMuteStateFunctions[CATEGORY_SINK] = &CAmPolicyEngine::_findSourceMuteState;
...
am_Error_e CAmPolicyEngine::processTrigger(gc_triggerParams_s& triggerParams)
{
	...
	else if (triggerParams.triggerType == USER_SET_SOURCE_MUTE_STATE)
    {
        gc_Source_s source;
        if (E_OK == mpConfigReader->getElementByName(triggerParams.sourceName, source))
        {
...
//find the mute state of source
am_Error_e CAmPolicyEngine::_findSourceMuteState(const gc_ConditionStruct_s &conditionInstance,
                                               std::vector<std::string > &listOutputs,
                                               const gc_triggerParams_s &parameters,
                                               const bool isLHS)
{
    std::string mandatoryParameter = parameters.sourceName;
    return _findMuteState(conditionInstance, listOutputs, mandatoryParameter, isLHS, ET_SOURCE);
}
